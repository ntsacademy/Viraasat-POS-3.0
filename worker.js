...
async function getStock(db) {
  ...
  const result = await db.prepare(`SELECT id, name, quantity, unit, low_stock_level, is_active, created_at FROM stock_items ORDER BY name`).all();
  return result.results || [];
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/health") return json({ success: true, application: "Viraasat POS Enterprise", status: "online", database: "connected" });
      ...
      // --- EXPENSES ROUTES ---
      if (path === "/api/expenses" && method === "GET") return json({ success: true, expenses: await getExpenses(db) });
      ...
      if (path === "/api/stock" && method === "POST") {
        await ensureSupportTables(db);
        const body = await request.json();
        const name = clean(body.name || body.item);
        if (!name) return json({ success: false, error: "Stock item name required" }, 400);
        
        await db.prepare(`
          INSERT INTO stock_items (name, quantity, unit, low_stock_level, is_active) VALUES (?, ?, ?, ?, 1)
          ON CONFLICT(name) DO UPDATE SET quantity=excluded.quantity, unit=excluded.unit, low_stock_level=excluded.low_stock_level, is_active=1, updated_at=CURRENT_TIMESTAMP
        `).bind(name, num(body.quantity ?? body.qty), clean(body.unit) || "pcs", num(body.low_stock_level, 5)).run();
        return json({ success: true });
      }
